...
func TestClient_NewClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			logger := zap.NewNop()
			client, err := NewClient(tt.url, "", "", "$G", authHandler, logger)
...
`
	...
	client, err := NewClient("nats://localhost:4222", credsFile.Name(), "", "$G", authHandler, logger)
	if err != nil {
...
func TestClient_WithInvalidCredentialsFile(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			client, err := NewClient("nats://localhost:4222", tt.credsFile, "", "$G", authHandler, logger)
...
func TestClient_PathTraversalProtection(t *testing.T) {
	...
	for _, path := range suspiciousPaths {
		t.Run(path, func(t *testing.T) {
			client, err := NewClient("nats://localhost:4222", path, "", "$G", authHandler, logger)
...
func TestClient_WithEmptyCredentialsFile(t *testing.T) {
	...
	client, err := NewClient("nats://user:pass@localhost:4222", "", "", "$G", authHandler, logger)
	if err != nil {
